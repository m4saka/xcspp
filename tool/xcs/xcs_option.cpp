...
namespace xcspp::tool::xcs
{
    ...
    void AddXCSOptions(cxxopts::Options & options)
    {
        ...
        options.add_options("XCS parameter")
            ...
            ("theta-del", "The experience threshold over which the fitness of a classifier may be considered in its deletion probability", cxxopts::value<std::uint64_t>()->default_value(std::to_string(defaultParams.thetaDel)), "THETA_DEL")
            ...
            ("theta-sub", "The experience of a classifier required to be a subsumer", cxxopts::value<std::uint64_t>()->default_value(std::to_string(defaultParams.thetaSub)), "THETA_SUB")
